...
class AppModel extends Model {
	...
	/**
	 * Find a record by the model's primary key.
	 *
	 * @param string|int $id The value of the primary key for the record to fetch.
	 * @param array $options Optional options
	 * @return mixed
	 * @access public
	 */
	public function get($id, $options = array()) {
		$options = Hash::merge($options, array('conditions' => array($this->alias . '.' . $this->primaryKey => $id)));
	 ...
	 * @param bool $excludeExisting If true, and if $this->data[Model][id] is present, will exclude that existing record from the check. When this is false, attempting to update an existing record will cause this method to fail (because the existing copy of the record with the matching field IS in the database.)
	 * @return bool Returns true if there are no existing records with a matching $check field.
	 * @access public
	 */
	 ...
	 * @return bool Returns true if the $check value is `null`.
	 * @access public
	 */
	 ...
	 *
	 * @param mixed $config The name of the CakeEmail config class to use.
	...
	/**
	 * Runs any fields present in $trimFields array through PHP's trim()
	 * function.
	 *
	 * @param array $trimFields The fields to trim
	 * @return bool true
	 */
	public function trim($trimFields = array()) {
		foreach ($trimFields as $trimField) {
			if (!empty($this->data[$this->alias][$trimField])) {
				$this->data[$this->alias][$trimField] = trim($this->data[$this->alias][$trimField]);
			}
		}
		return true;
	}
	...
	/**
	 * Checks for required configure variables and returns a new Payment
	 * object.
	 *
