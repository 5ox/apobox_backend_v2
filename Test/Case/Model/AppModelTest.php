...
class AppModelTest extends CakeTestCase {
	...
	/**
	 * Confirm that AppModel::trim trims only the fields it's supposed to.
	 *
	 * @return void
	 */
	public function testTrim() {
		...
		$result = $this->AppModel->trim($trimFields);
		$this->assertTrue($result);
		$this->assertSame($expected, $this->AppModel->data);
	}

	/**
	 * Confirm getPaymentLib creates an instance of `Payment`
	 *
	 * @return void
	 */
	public function testGetPaymentLib() {
		$Model = new TestAppModel();
		$this->assertInstanceOf('Payment', $Model->getPaymentLib());
	}

	/**
	 * Confirm getPaymentLib throws an exception if config vars are missing
	 *
	 * @return void
	 */
	public function testGetPaymentLibMissingConfig() {
		Configure::write('PayPal.clientId', null);
		$Model = new TestAppModel();
	...
	/**
	 * Confirm that the model::find method is called with the expected options.
	 *
	 * @return void
	 */
	public function testGet() {
		$id = 1;
		$options = ['contain' => ['Foo']];
		$this->AppModel = $this->getMockBuilder('AppModel')
			->setMethods(['find'])
			->getMock();
		...
		$this->AppModel->expects($this->once())
			...
			->with(
				...
				$this->logicalAnd($this->arrayHasKey('contain'), $this->arrayHasKey('conditions'))
			)
			->will($this->returnValue('canary'));

		$result = $this->AppModel->get($id, $options);

		$this->assertSame('canary', $result);
	}

	/**
	 * Confirm getDateTime() can return an instance of DateTime.
	 *
	 * @return void
	 */
	public function testGetDateTime() {
		$Model = new TestAppModel();
		$result = $Model->getDateTime();

		$this->assertInstanceOf('DateTime', $result);
	}

	/**
	 * Confirm taskFactory() can return an instance of QueuedTask.
	 *
	 * @return void
	 */
	public function testTaskFactory() {
		$result = $this->AppModel->taskFactory();

		$this->assertInstanceOf('QueuedTask', $result);
	}
}
